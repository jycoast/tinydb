  ...
  <div class="table-data-page">
    <div v-if="columnNames.length > 0" class="filter-section">
      ...
      <div class="filter-actions">
        ...
        <el-button type="primary" size="small" :loading="loading" @click="applyFilter"
          >查询</el-button
        >
    ...
    >
      <div class="context-menu-item" @click="handleCopyAsInsert"> 复制为Insert语句 </div>
    </div>
    ...
    <div class="pagination-row">
      <div class="pagination-left">
        ...
        <span v-if="selectedColumnDataIndex" class="column-copy-hint">
          已选中列「{{ selectedColumnDataIndex }}」，按 Ctrl+C 复制该列
        </span>
      </div>
      <el-pagination
...
<script lang="ts" setup>
  ...
  const selectedColumnDataIndex = ref<string | null>(null);
  ...
  const tableColumns = computed(() => {
    ...
    if (
      ...
      tableStructure.value.columns &&
      Array.isArray(tableStructure.value.columns)
    ) {
  ...
  const tableRows = computed(() => {
    return rows.value.map((r: any, i: number) => ({
      key: (currentPage.value - 1) * pageSize.value + i,
      ...r,
    }));
  });
  ...
  const summaryText = computed(() => {
    ...
    if (hasMore.value && rows.value.length === pageSize.value)
      return `第 ${from}-${to} 条（可能有更多）`;
  ...
  async function loadTableStructure() {
    ...
    try {
      ...
      if (structureRef.value && structureRef.value.tables) {
        ...
        const table = structureRef.value.tables.find(
          (t: any) =>
  ...
  function handleHeaderClick(columnId: string) {
    selectedColumnDataIndex.value = selectedColumnDataIndex.value === columnId ? null : columnId;
  }

  function copySelectedColumn() {
    if (!selectedColumnDataIndex.value || tableRows.value.length === 0) return;
    const text = tableRows.value
      .map((r) => formatCellValue(r[selectedColumnDataIndex.value!]))
      .join('\n');
    navigator.clipboard
      .writeText(text)
      .then(() => {
        ElMessage.success(`已复制该列 ${tableRows.value.length} 条数据`);
      })
      .catch(() => {
  ...
  function handleRowRightClick(row: any, event: MouseEvent) {
  ...
  function isCellEditing(rowIndex: number, columnIndex: number): boolean {
    return (
      editingCell.value?.rowIndex === rowIndex && editingCell.value?.columnIndex === columnIndex
    );
  }
  ...
  function handleKeydown(e: KeyboardEvent) {
    if ((e.ctrlKey || e.metaKey) && e.key === 'c' && selectedColumnDataIndex.value) {
      copySelectedColumn();
      e.preventDefault();
    }
  }
